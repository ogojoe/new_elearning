    ...
    @foreach ($section->lessons as $item)
        <article class="card mt-4">
            <div class="card-body">
                @if ($lesson->id == $item->id)
                    <div>
                        ...
                        <div class="mt-4 flex justify-end">
                            ...
                            <button class="btn btn-danger ml-2" wire:click="cancel" >Cancelar</button>
                        </div>
                ...
                @else
                    <header><h1><i class="far fa-play-circle text-blue-500 mr-1"></i> Lección: {{$item->name}}</h1></header>
        ...
        <article class="card" x-show="open">
            <div class="card-body">
                ...
                <div class="mb-4">
                    ...
                    <div class="flex items-center mt-4">
                        <label class="w-32">Plataforma:</label>
                        <select wire:model="platform_id" class="block appearance-none w-full text-base border border-gray-200 rounded focus:bg-white focus:border-gray-500">
                            @foreach ($platforms as $platform)
                                <option value="{{$platform->id}}">{{$platform->name}}</option>
                            @endforeach
                        </select>
