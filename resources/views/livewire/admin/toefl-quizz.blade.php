<div>
    <h1 class="text-2xl font-bold">Bateria de preguntas de evaluación {{$toefl->name}}</h1>
    <hr class="mt-2 mb-6">

    <h1 class="text-2xl font-bold">Grammar</h1>
    <hr class="mt-2 mb-6">

    @foreach ($toefl->questions->where('skill_id', 1) as $item)
    <article class="card mb-6" x-data="{open: false}" >
        <div class="card-body bg-gray-100" x-on:click.outside="open = false">
            ...
            <div x-show="open">
                @livewire('admin.toefl-quizz-answers', ['question' => $item], key('quizz-answer'.$item->id))
            </div>
    ...
    @foreach ($toefl->questions->where('skill_id', 2) as $item)
    <article class="card mb-6" x-data="{open: false}" >
        <div class="card-body bg-gray-100" x-on:click.outside="open = false">
            @if ($question->id == $item->id)
                <form wire:submit.prevent = "update">
            ...
            <div x-show="open">
                @livewire('admin.toefl-quizz-question-resource', ['question' => $item], key('question-resource' . $item->id))
            </div>
    ...
    @foreach ($toefl->questions->where('skill_id', 3) as $item)
    <article class="card mb-6" x-data="{open: false}" >
        <div class="card-body bg-gray-100" x-on:click.outside="open = false">
            @if ($question->id == $item->id)
                <form wire:submit.prevent = "update">
